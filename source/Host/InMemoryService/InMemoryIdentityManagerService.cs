...
{
	internal class InMemoryIdentityManagerService : IIdentityManagerService
	{
		...
		private IdentityManagerMetadata GetMetadata()
		{
			if (metadata == null)
			{
				var createprops = new List<PropertyMetadata>
				{
					PropertyMetadata.FromProperty<InMemoryUser>(x => x.Username, Constants.ClaimTypes.Username, required: true)
				};

				var updateprops = new List<PropertyMetadata>();
				updateprops.AddRange(new[]
				{
					PropertyMetadata.FromProperty<InMemoryUser>(x => x.Username, Constants.ClaimTypes.Username, required: true),
					PropertyMetadata.FromPropertyName<InMemoryUser>("Password", Constants.ClaimTypes.Password, required: true),
					PropertyMetadata.FromFunctions<InMemoryUser, string>(Constants.ClaimTypes.Name, GetName, SetName, "Name", required: true)
				});
				...
				roleUpdateProps.Add(PropertyMetadata.FromProperty<InMemoryRole>(x => x.Description, "description"));
		...
		private IEnumerable<string> ValidateUserProperty(string type, string value)
		{
			switch (type)
			{
				...
				{
					if (users.Any(x => x.Username == value))
						return new[] {"That Username is already in use"};
				}
					break;
